...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Convert SQL to lowercase for easier detection
    ...
    //      OR 'string'='string'
    //
    // These are the exact patterns used by run_query_injection().
    // -------------------------------------------------------------------------

    // Regex for numeric OR injection (e.g., "or 1=1", "or 2=2")
    std::regex numericPattern(R"(or\s+[0-9]+\s*=\s*[0-9]+)");

    // Regex for quoted string OR injection (e.g., "or 'hi'='hi'")
    std::regex stringPattern(R"(or\s*'[^']+'\s*=\s*'[^']+')");

    // Check for either pattern
    if (std::regex_search(lowered, numericPattern) ||
        std::regex_search(lowered, stringPattern))
    {
        std::cout << "SQL Injection Attempt Blocked!" << std::endl;
        std::cout << "Rejected SQL: " << sql << std::endl;
        return false;  // Prevent execution
    }

    // -------------------------------------------------------------------------
    // If no injection detected, execute normally
    // -------------------------------------------------------------------------
    char* error_message;
